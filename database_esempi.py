...
LEFT JOIN (
SELECT q.driverId, COUNT(*) AS convertite, FROM qualifying AS q, 
JOIN races AS r ON q.raceId = r.raceId AND r.year   = 2012
JOIN results AS re ON re.raceId  = q.raceId AND re.driverId = q.driverId AND re.position = 1    -- vittoria
WHERE q.position = 1 -- pole position
GROUP BY q.driverId) AS p ON q.driverId = p.driverId
WHERE q.position = 1   -- pole position nella gara
GROUP BY q.driverId, d.driverRef
ORDER BY pole DESC
"""